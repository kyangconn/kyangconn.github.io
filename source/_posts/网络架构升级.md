...
```mermaid
graph TD
    User[用户浏览器] -->|HTTPS| ECS[阿里云 ECS<br/>Nginx+FRPS]

    subgraph 公网服务器
        Nginx_ECS[Nginx<br/>80/443 端口]
        FRPS[frps<br/>7000 端口]
        Nginx_ECS -->|转发 | FRPS
    end
    ...
    subgraph 家庭内网
        FRPC --> Traefik[Traefik 容器<br/>Docker 网关]
        Traefik --> Gitea[Gitea:3000]
        Traefik --> Homarr[Homarr:7577]
        Traefik --> Uptime[Uptime Kuma]
        Traefik --> Overleaf[Overleaf:3001]
    end
```
